...
import { CalendarCheck, CalendarDays, Clock, FileText, Flag, Layers, ListChecks, Repeat, Save, Timer, User, X } from "lucide-react";
...
function CreateGeneralTaskMasterModal({
    ...
    const { taskPriority, isLoading: isLoadingForaskPriority } =
        useTaskPriority();
    const { taskType, isLoading: isLoadingForTaskType } =
        useTaskType();
    const [isSubmitting, setIsSubmitting] =
        useState<boolean>(false);
    const { taskFrequency, isLoading: isLoadingForTaskFrequency } = useTaskFrequency();
    const { intervalList, isLoading: isLoadingForTaskFrequencyInterval } = useTaskFrequencyInterval();
    ...
    const createMyTask = async () => {
        ...
        await axiosClient
            .post(POST_API.CREATE_GENERAL_TASK_MASTER, postData, { withCredentials: true })
            .then((response) => {
    ...
    return (
        <FormLayout widthPercent={95} padding={3}>
            ...
            <div
                className={`${isSubmitting ? "cursor-wait" : "cursor-default"}`}
            >
                <FormHeader
                    icon={ListChecks}
                    onClose={() => {
                        ...
                        <form className="space-y-4 mt-2">
                            <div className="grid grid-cols-3 gap-2">
                                {!isLoadingForTaskType ? (
                                    <div className="">
                                        <CustomDropdown
                                            ...
                                            options={taskType!}
                                        />
                                        {errors.taskType && <span className="mt-1 ml-1 text-red-500 caption-custom-inactive">{errors.taskType}</span>}
                                    </div>
                                ) : (
                                ...
                                {!isLoadingForaskPriority ? (
                                    <div className="">
                                        ...
                                        />
                                        {errors.taskPriority && <span className="mt-1 ml-1 text-red-500 caption-custom-inactive">{errors.taskPriority}</span>}
                                    </div>
                                ) : (
                                    <div className="space-y-1 animate-pulse">
                                        {/* Label skeleton */}
                                ...
                                {!isLoadingForTaskFrequency ? (
                                    <div className="">
                                        ...
                                        />
                                        {errors.frequency && <span className="mt-1 ml-1 text-red-500 caption-custom-inactive">{errors.frequency}</span>}
                                    </div>
                                ) : (
                                    <div className="space-y-1 animate-pulse">
                                        {/* Label skeleton */}
                                ...
                                {!isLoadingForTaskFrequencyInterval ? (
                                    <div className="">
                                        <CustomDropdown
                                            ...
                                            onSelect={(v) => handleDropdownChange("frequencyInterval", v)}
                                        />
                                        {errors.frequencyInterval && <span className="mt-1 ml-1 text-red-500 caption-custom-inactive">{errors.frequencyInterval}</span>}
                            ...
                            <div className="grid grid-cols-1">
                                ...
                                <span className="caption-custom">
                                    <span className="">Note :</span> If a task
                                    assign to is not selected or is removed, then task will
                                    assigned to
