...
function LeadTasksModal() {
  ...
  const [leadTaskChangeCount , setLeadTaskChangeCount] = useState<number>(0);
   ...
   const getLeadActivity = async() => {
    ...
    await axios.post(POST_API.GET_LEAD_ACTIVITY,getLeadActivityPostData,{
      withCredentials : true
    })
    .then((response)=>{
     if(response.status === STATUS_CODE.OK){
      response.data.map((res : any) => {
        setLeadActivity((prev) => [...prev,{
          id:res.id,
          name : res.name,
      ...
      if(error.status === STATUS_CODE.UNATHORISED){
        const refreshTokenResponse = await RefreshToken({callFunction : getLeadActivity})
    ...
    await axios.post(POST_API.GET_LEAD_TASK_PRIORITY,getLeadTaskPriorityPostData,{
      withCredentials : true
    })
    .then((response)=>{
     if(response.status === STATUS_CODE.OK){
      response.data.map((res : any) => {
        setLeadTaskPriority((prev) => [...prev,{
          id:res.id,
          name : res.name,
          isActive : res.isactive
        }])
      ...
      if(error.status === STATUS_CODE.UNATHORISED){
        const refreshTokenResponse = await RefreshToken({callFunction : getLeadTaskPriority})
        if(refreshTokenResponse){
          getLeadTaskPriority();
        }
      }
    ...
    await axios.post(POST_API.GET_LEAD_TASK_STAGE,getLeadTaskStagePostData,{
      withCredentials : true
    })
    .then((response)=>{
     if(response.status === STATUS_CODE.OK){
      response.data.map((res : any) => {
        setLeadTaskStage((prev) => [...prev,{
          id:res.id,
          name : res.name,
          isActive : res.isactive
        }])
      ...
      if(error.status === STATUS_CODE.UNATHORISED){
        const refreshTokenResponse = await RefreshToken({callFunction : getLeadTaskStage})
        if(refreshTokenResponse){
          getLeadTaskStage();
        }
      }
...
const getLeadTasks = async() => {
  ...
  const getLeadTaskPostData = {
    ...
    lead_task_priority_id : leadTaskPriorityId === 0 ? null : leadTaskPriorityId,
    lead_task_stage_id : leadTaskStageId === 0 ? null : leadTaskStageId, 
    requestedby_id : loginStatus.id
  }
  await axios.post(POST_API.GET_LEAD_TASK,getLeadTaskPostData,{
    withCredentials:true
  }).then((response) => {
    if(response.status === STATUS_CODE.OK){
      response.data.map((res : any) => {
    ...
    if(error.status ===STATUS_CODE.UNATHORISED){
      const refreshTokenResponse = await RefreshToken({callFunction:getLeadTasks})
      if(refreshTokenResponse){
        getLeadTasks();
      }
    }
...
useEffect(() => {
...
},[leadData,leadTaskStageId,leadActivityId,leadTaskPriorityId,leadTaskChangeCount])
  ...
  const [isCreateLeadTaskModalOpen,setIsCreateLeadTaskModalOpen] = useState<boolean>(false);
  return (
            ...
            <div className="flex justify-between  w-full pr-3">
              ...
              {visibleAssignUsersBtn && (
                ...
                <button
                  ...
                  onClick={()=>{setIsCreateLeadTaskModalOpen(true)}}
                >
                  <Plus size={10} />
                </button>
              </div>
