...
function GoogleOAuthConsent() {
  ...
  const state = searchParams.get("state");
    ...
    const handleConfirm = () =>{
      localStorage.setItem(LOCALSTORAGE_KEYS.REDIRECT_PLATFORM,STRING_VALUES.GOOGLE_MEET)
    const baseUrl =
   ...
   useEffect(() => {
      if (code && state){
        ...
        if (!googleMeetStatus.isConnected && redirectPlatform === STRING_VALUES.GOOGLE_MEET) {
  ...
  return (createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-lg shadow-xl w-full max-w-md mx-4 p-6 animate-in fade-in zoom-in duration-200">
        ...
        <div className="mb-6">
          <h2 className="section-header-custom mb-2">Sign In With Google</h2>
          <p className="input-label-custom">Do you want to signin with your google account for managing meetings</p>
        </div>
