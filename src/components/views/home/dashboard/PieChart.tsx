...
const PieChart = ({
  ...
  const totalLeads = data ? data!.reduce((sum, item) => sum + item.total, 0) : 0;
  ...
  const transformedData = data ? data.map((item, index) => ({
    ...
    percentage: totalLeads !== 0 ? Math.round((item.total / totalLeads) * 100) : 0,
  })) : [];

  // Filter out items with 0 count for better visualization
  const filteredData = transformedData ? transformedData.filter(item => item.count > 0) : [];

  // Calculate angles for pie slices
  let currentAngle = -90; // Start from top

  const pieSlices: PieSlice[] = filteredData ?  filteredData.map((item) => {
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-600">
            {chartFor === "leadByStatus" ? "Distribution of leads by current status" : "Distribution of leads by acquisition source" }
          </p>
        </div>
      </div>
      ...
      {filteredData.length > 0 ? (
           ...
           console.log(pieSlices)
            ...
            <svg width="300" height="300" className="transform rotate-0">
              {pieSlices.map((slice, index) => (
                <path
                  key={slice.sourceId}
                  d={createArcPath(150, 150, 120, slice.startAngle, slice.endAngle)}
                  fill={slice.color}
                  stroke="white"
                  strokeWidth="2"
        ...
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="text-gray-500 font-medium">No lead source data available</p>
          </div>
        </div>
      )}
