...
import LookupCompanyUserAgGrid from "../../../ag-grid/lookup-grids/LookupCompanyUserAgGrid";
...
function LookupCompanyUserSelection({
  isOpen,
  onClose,
    ...
    try {
      ...
      if (error.status === STATUS_CODE.UNATHORISED) {
        const refreshTokenStatus = await RefreshToken({
          callFunction: fetchLookupCompanyUsers,
        });
        if (refreshTokenStatus) {
          fetchLookupCompanyUsers();
        }
      } else {
      ...
      <div className="fixed inset-0 z-50 bg-black bg-opacity-5 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-lg p-3 w-full max-w-5xl max-h-[100vh] overflow-y-auto relative animate-fadeIn">
          <FormHeader
            icon={User}
            onClose={onClose}
            preText={preText ?? "Select Company User"}
            description={description ?? "Select the user "}
          />
...
export default LookupCompanyUserSelection;
