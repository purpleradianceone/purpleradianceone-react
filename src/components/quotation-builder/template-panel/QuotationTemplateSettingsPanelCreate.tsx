...
import { HEADER_STORAGE_KEY } from "../blocks/HeaderBlockQuotation";
import { FOOTER_STORAGE_KEY } from "../blocks/FooterBlockQuotation";
import { PageBlockLayoutKey } from "../blocks/PageBlockQuotation";
...
> = ({ htmlTemplateTypeSubjectPlaceholder }) => {
  ...
  function getCraftJson(): string {
        const storedDefaultHeader = localStorage.getItem(HEADER_STORAGE_KEY + loginStatus.id);
        const storedDefaultFooter = localStorage.getItem(FOOTER_STORAGE_KEY + loginStatus.id);
        const storedDefaultPageLayout = localStorage.getItem(PageBlockLayoutKey + loginStatus.id);
      const jsonFileData = {
  ...
  const createEmailTemplateCreate = (resultJson: string) => {
    try {
        ...
        >
          <form
            onSubmit={(e) => {
              ...
              createEmailTemplateCreate(resultJson);
            }}
          >
            <FormHeader
              icon={LucideMail}
            ...
            <div className="flex flex-col gap-4 mt-2">
              ...
              <FormInput
                ...
                placeholder={
                  htmlTemplateTypeSubjectPlaceholder ?? "Enter template name"
                }
              />
              ...
              <TextAreaInput
                ...
                placeholder="Enter description" rows={2} cols={0}              />
