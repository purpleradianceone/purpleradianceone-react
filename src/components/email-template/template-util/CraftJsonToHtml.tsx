...
export function craftJsonToHtml(json: string): string {
  try {
    ...
    return `<div class="craft-error">Error rendering content: ${
...
function renderNode(data: CraftData, nodeId: string): string {
  ...
  const renderLinkedNodes = (linkedNodes?: Record<string, string>) =>
    linkedNodes
      ? Object.values(linkedNodes)
          .map((id) => renderNode(data, id))
          .join("")
      : "";
...
const defaultRenderer: ComponentRenderer = ({
  node,
  renderChildren,
  renderLinkedNodes,
}) => {
...
const componentRenderers: Record<string, ComponentRenderer> = {
  ...
  div: ({ node, renderChildren, renderLinkedNodes }) => {
    ...
    const attrs = Object.entries(node.props)
      .filter(
        ([key]) => !["style", "nodes", "linkedNodes", "isCanvas"].includes(key)
      )
  ...
  ColumnBlock: ({ node, data }) => {
    const { gap = "10px", direction = "row", style = {} } = node.props;
    ...
    const columns = linkedNodeEntries
      .map(([_, columnId], _index, arr) => {
        const colWidth = `calc(${100 / arr.length}% - ${
          (parseFloat(gap) * (arr.length - 1)) / arr.length
        }px)`;
        ...
        return `<div style="${columnWrapperStyle}">${renderNode(
          data,
          columnId
        )}</div>`;
      })
  ...
  LexicalText: ({ node }) => {
    try {
      ...
      const editorState =
        typeof node.props.editorState === "string"
...
function extractHtmlFromLexical(editorState: any): string {
  ...
  return `<p${styleAttr}>${children}</p>`;
    ...
    if (node.type === "heading") {
      ...
      if (node.format) {
        if (node.format & (1 << 0))
          style = appendStyle(style, "text-align: left;");
        if (node.format & (1 << 1))
          style = appendStyle(style, "text-align: center;");
        if (node.format & (1 << 2))
          style = appendStyle(style, "text-align: right;");
        if (node.format & (1 << 3))
          style = appendStyle(style, "text-align: justify;");
      }
