...
const Button: React.FC<ButtonProps> = (prop) => {
  ...
  if(prop.disabled){
    return (
      ...
      className = {prop.className ? "cursor-not-allowed opacity-50 "+prop.className: (prop.children == "Reset Password" ? "mt-6 " : "") +"w-full flex justify-center py-2.5 px-4 cursor-not-allowed opacity-50  border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500" } 
        >
        {prop.children}
      </button>
    );
