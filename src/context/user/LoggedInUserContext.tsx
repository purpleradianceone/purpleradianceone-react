...
import {
  LoggedInUserContextProps,
  LoggedInUserType,
} from "../../@types/company-users/LoggedInUserContextType";
import { DATA_TYPE } from "../../constants/AppConstants";

const LoggedInUserContext = createContext<LoggedInUserContextProps | undefined>(
  undefined
);
...
export const LoggedInUserContextProvider: React.FC<{ children: ReactNode }> = ({
  ...
  useEffect(() => {
    localStorage.setItem(LOCALSTORAGE_KEYS.LOGIN_STATUS,
      JSON.stringify(loginStatus)
    );
    localStorage.setItem("companyId", loginStatus.companyId.toString()); //for api call blocking after logout
  }, [loginStatus]);
